...
async function checkPasskeyStatus() {
  try {
    ...
    console.error(error);
    toast.error("Erro ao verificar status da passkey");
  } finally {
    loading.value = false;
  }
}
...
async function createPasskey() {
  try {
    ...
    console.error(error);
    toast.error(error.data?.statusMessage ?? "Erro ao criar passkey");
  } finally {
...
async function deletePasskey() {
  try {
    ...
    toast.success("Passkey deletada com sucesso");
    await checkPasskeyStatus();
  } catch (error) {
    console.error(error);
    toast.error(error.data?.statusMessage ?? "Erro ao deletar passkey");
  } finally {
